...
class AndValidator extends Validator {
	...
	toJS() {
		...
		if(this._validatorArray.flag)
			result['validators'] = this._validatorArray.value.map(validator => validator.toJS());
		...
		if(schemaType === 'and') {
			...
			if(schemaMap.has('validators')) {
				...
				this.validators(schemaMap.get('validators').map(validator => {
					...
					return (new this._validatorMap.get(schemaType)(this._validatorMap)).fromJS(validator);
				}));
			}
